...
import { useParams } from 'next/navigation'; // Use to access dynamic params
...
const ArticleDetail = () => {
  ...
  const title = params.title; // Access the dynamic title slug
  const article = articles.find((a) => slugify(a.title) === title);

  if (!article) {
    notFound(); // Handle invalid title slug
    return null;
  }
