      ...
      <div className="pt-16 w-full">
        ...
        >
          {tripled.map((item, idx) => (
            ...
            >
              <div
                className={`flip-card-inner relative w-full h-full rounded-2xl transition-transform duration-500 ${flippedCards[idx] ? "rotate-y-180" : ""}`}
              >
                {/* Front of card (image) */}
                ...
                <div className="flip-card-back absolute w-full h-full rounded-2xl bg-black text-white p-6 flex flex-col justify-center">
                  ...
                  <p className="text-gray-300 whitespace-normal break-words overflow-hidden">{item.description}</p>
                </div>
              </div>
            </div>
          ))}
